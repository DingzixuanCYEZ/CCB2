...
export const StudySession: React.FC<StudySessionProps> = ({ deck, onUpdateDeck, onExit, onTimeUpdate, onSessionComplete }) => {
  ...
  // src/components/StudySession.tsx (Part 2 - 完结)
    ...
    if (deck.coolingPool && deck.coolingPool.length > 0) {
      return (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-slate-50 z-[100] p-4 animate-in fade-in">
          <div className="bg-white p-6 rounded-3xl shadow-xl text-center max-w-sm w-full border border-slate-100 animate-in zoom-in-95">
            ...
            }} className="py-3.5 text-base font-black bg-sky-500 border-0 text-white">立即全部唤醒</Button>
            <Button fullWidth variant="ghost" onClick={handleRequestExit} className="mt-2 text-xs text-slate-400">退出查看报告</Button>
          </div>
        </div>
      );
    }
    return <div className="fixed inset-0 flex flex-col items-center justify-center bg-white z-[100]"><AlertCircle className="w-12 h-12 text-rose-500 mb-2"/><h2 className="text-lg font-black text-slate-800">数据加载异常</h2><Button onClick={onExit} className="mt-4 px-6 py-2">返回首页</Button></div>;
  }
  ...
  const currentLabels = (activeScore === undefined || activeScore === 0) 
            ...
            {deck.queue.map((id, idx) => {
              ...
              return (
                <div key={id} className={`flex items-center justify-between text-[11px] py-1.5 px-2.5 rounded-xl border transition-all ${isCurrent ? 'bg-indigo-50 border-indigo-200 shadow-sm scale-[1.02] z-10' : 'bg-white border-transparent hover:bg-slate-50'}`}>
                  ...
                  <div className="px-1.5 py-0.5 rounded-md text-[8px] font-black text-white shrink-0 ml-2 shadow-sm" style={{backgroundColor: getScoreBadgeColor(p.score)}}>{getPhraseTag(p.score)}</div>
                </div>
              )
            })}

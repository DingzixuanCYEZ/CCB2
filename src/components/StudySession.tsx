// src/components/StudySession.tsx (Part 1/3)
...
import { 
  ...
  Edit2, BarChart2, ListOrdered, X, Trophy, StickyNote, 
...
export const StudySession: React.FC<StudySessionProps> = ({ deck, onUpdateDeck, onExit, onTimeUpdate, onSessionComplete }) => {
  ...
  // src/components/StudySession.tsx (Part 2/3)
        ...
        <div className="max-w-2xl w-full bg-white rounded-3xl shadow-2xl p-6 sm:p-8 flex flex-col space-y-6 my-4 sm:my-auto border border-slate-100">
          ...
          {sessionResults.length > 0 && (
            <div className="border-t border-slate-100 pt-6">
               <h3 className="text-sm font-black text-slate-800 mb-4 flex items-center gap-2"><ListOrdered size={16} className="text-indigo-500"/> 详细复盘 (按表现排序)</h3>
          ...
          <Button fullWidth onClick={handleFinalExit} className="py-4 text-lg font-black rounded-2xl shadow-xl mt-4 bg-indigo-600 border-0 text-white hover:bg-indigo-700">确认返回主页</Button>
        </div>
      </div>
    );
  };
    ...
    <div className="fixed inset-0 bg-slate-50 z-[100] flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-white shadow-sm shrink-0 relative z-[60]">
        <div className="flex items-center justify-between px-3 py-1.5 h-12">
          ...
          <div className="flex-1 flex flex-col justify-center items-center max-w-[70%]">
              <div className="flex justify-between items-end w-full max-w-[240px] mb-1">
                <div className="flex items-center gap-1.5 truncate">
                   <span className="text-[10px] font-black text-indigo-600">{(liveMasteryValue).toFixed(2)}%</span>
                   <span className="text-[9px] text-slate-300 font-bold truncate">/ {deck.name}</span>
                </div>
                <span className="text-[9px] font-mono font-bold text-slate-400">{new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
              </div>
              <div className="h-1.5 w-full max-w-[240px] bg-slate-100 rounded-full overflow-hidden relative border border-slate-50">
              ...
              <div className="flex justify-between items-start w-full max-w-[240px] mt-1 leading-none">
                ...
                <span className="text-[9px] font-bold flex items-center gap-1">
                  ...
                  <span className="text-amber-500">{stats.count2_3}</span>
                  <span className="text-slate-200">/</span>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6">
            ...
            <div className="border-t border-slate-50 pt-5">
               ...
               <div className="grid grid-cols-2 gap-2">
                 ...
                 .map(([tag, count]) => {
                   // 核心修复：解析 tag 字符串，为 getScoreBadgeColor 提供正确数值染色
        ...
        <div className={`flex-1 flex flex-col items-center p-2 sm:p-4 transition-all duration-300 ${showQueue ? 'lg:pr-[280px]' : ''} ${showStats ? 'lg:pl-[280px]' : ''}`}>
          <div className="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-slate-100 flex flex-col h-full max-h-[600px] overflow-hidden relative">
            ...
            {!isEditing && phase !== 'QUESTION' && (
              <button onClick={() => setIsEditing(true)} className="absolute top-2.5 right-2.5 z-10 p-2 text-slate-300 hover:text-indigo-500 transition-all active:scale-90 bg-white/60 rounded-xl shadow-sm"><Edit2 size={16}/></button>
            )}
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-8 flex flex-col items-center w-full relative">
                ...
                {phase === 'ANSWER' && (
                  <div className="w-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300 pb-2 mt-auto">
                    <div className="text-center py-3 px-4 rounded-xl w-full mb-5 bg-indigo-50/50 shadow-sm border border-indigo-100/50">
                    ...
                    <div className="w-full mt-auto space-y-4">
                      ...
                      {/* 难度评价 (继承上次) */}
                      <div className="bg-slate-50/80 p-2 rounded-xl border border-slate-100 flex justify-between items-center gap-3 shadow-sm">
                            ...
                            {[0, 1, 2, 3, 4, 5].map(v => {
                              ...
                              return (
                                <button key={v} disabled={disabled} onClick={() => setProf(v)} 
                                  className={`flex flex-col items-center justify-center p-1.5 sm:p-2.5 rounded-xl border-2 transition-all group ${disabled ? 'opacity-20 grayscale bg-slate-50 border-slate-100' : prof === v ? 'bg-emerald-50 border-emerald-500 scale-105 z-10 shadow-md' : 'bg-white border-slate-100 hover:border-emerald-300'}`}>
                      ...
                      <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 shadow-sm animate-in slide-in-from-bottom-2 mt-2 w-full">
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex flex-col">
                              ...
                              <div className="text-[11px] font-bold mt-1.5 flex items-center gap-1.5">
                                <span className="text-slate-500">Score 轨迹:</span>
                        ...
                        <div className="px-1">
                            ...
                            <div className="flex justify-between text-[8px] font-black text-slate-400 mt-2 tracking-widest uppercase">
                              <span>NEAR</span>
                              {isNowFrozen ? (
                                  <span className="text-sky-500 font-bold flex items-center gap-1 animate-pulse"><Waves size={10}/> 将在队尾冻结冷却 (+{currentBackDisplay - (deck.queue.length-1)}步)</span>
                              ) : (
